...
import { EntityArrayResponseType, SocietyService } from '../service/society.service';
...
export class SocietyComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISociety): number => this.societyService.getSocietyIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.societies = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISociety[] | null): ISociety[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.societyService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
