...
type RestOf<T extends IMember | NewMember> = Omit<T, 'dob' | 'applicationDate' | 'boardResolutionDate' | 'lastModified' | 'createdOn'> & {
  dob?: string | null;
...
export class MemberService {
  ...
  protected convertDateFromClient<T extends IMember | NewMember | PartialUpdateMember>(member: T): RestOf<T> {
    return {
      ...
      boardResolutionDate: member.boardResolutionDate?.format(DATE_FORMAT) ?? null,
  ...
  protected convertDateFromServer(restMember: RestMember): IMember {
    return {
      ...
      boardResolutionDate: restMember.boardResolutionDate ? dayjs(restMember.boardResolutionDate) : undefined,
