...
import { EntityArrayResponseType, MemberLandAssetsService } from '../service/member-land-assets.service';
...
@Component({
  selector: 'jhi-member-land-assets',
  templateUrl: './member-land-assets.component.html',
})
export class MemberLandAssetsComponent implements OnInit {
  ...
  trackId = (_index: number, item: IMemberLandAssets): number => this.memberLandAssetsService.getMemberLandAssetsIdentifier(item);
  ...
  delete(memberLandAssets: IMemberLandAssets): void {
    const modalRef = this.modalService.open(MemberLandAssetsDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.memberLandAssets = memberLandAssets;
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.memberLandAssets = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IMemberLandAssets[] | null): IMemberLandAssets[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    return this.memberLandAssetsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
